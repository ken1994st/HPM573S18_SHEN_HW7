...
class MultiCohort:
    ...
    def simulate(self, n_time_steps):
        ...
        for i in range(len(self._ids)):
            ...
            self.percentages.append(cohort.per_after_five_year())
...
data.sample_posterior()

print('Estimate of mortality probability ({:.{prec}%} credible interval):'.format(1-0.05, prec=0),
      data.get_mortality_estimate_credible_interval(0.05, 4))


Calimodel=MultiCohort(ids=data.cohortids,
            mortality_probs=data.mortalityresamples,
            pop_sizes=[POP_SIZE]*POST_N)
Calimodel.simulate(N_TIME_STEPS)

print("95% credible interval become shorter with larger sample in this case")
